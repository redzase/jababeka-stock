...
class Divisi extends MY_Controller 
{
    ...
    public function index($page = 1){
        ...
        $all_data = $this->Divisimodel->get_list($params);

        $params = array(
            "get_total" => TRUE,
            );
        $total = $this->Divisimodel->get_list($params);  
    ...
    private function _do_add() 
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $action = $this->Divisimodel->create($data_create);

            $result["status"]  = $action;
            $result["message"] = ($action) ? "Divisi successfully created." : "Divisi failed created.";
    ...
    private function _do_edit($id) 
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $action = $this->Divisimodel->update($id, $data_update);

            $result["status"]  = $action;
            $result["message"] = ($action) ? "Divisi successfully updated." : "Divisi failed updated.";
    ...
    public function edit($id)
    {
        ...
        $all_data = $this->Divisimodel->get_detail($params);
    ...
    {
        $action = $this->Divisimodel->delete($id);

        if ($action === TRUE) {
            $return = array(
                "status"  => TRUE,
                "message" => "Divisi successfully deleted.",
                );
        } else {
            $return = array(
                "status"  => FALSE,
                "message" => "Divisi failed deleted.",
                );
        }
