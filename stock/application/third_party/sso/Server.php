...
abstract class Server
{
    ...
    public function attach()
    {
        ...
        if (empty($_GET['broker'])) return $this->fail("No broker specified", 400);
        if (empty($_GET['token'])) return $this->fail("No token specified", 400);

        if (!$this->returnType) return $this->fail("No return url specified", 400);

        $checksum = $this->generateAttachChecksum($_GET['broker'], $_GET['token']);

        if (empty($_GET['checksum']) || $checksum != $_GET['checksum']) {
            return $this->fail("Invalid checksum", 400);
        }

        $this->startUserSession();
        $sid = $this->generateSessionId($_GET['broker'], $_GET['token']);
    ...
    protected function fail($message, $http_status = 500)
    {
        ...
        if ($this->returnType === 'jsonp') {
            echo $_GET['callback'] . "(" . json_encode(['error' => $message]) . ", $http_status);";
            exit();
        }

        if ($this->returnType === 'redirect') {
            $url = $_GET['return_url'] . '?sso_error=' . $message;
            header("Location: $url", true, 307);
            echo "You're being redirected to <a href='{$url}'>$url</a>";
            exit();
        }
