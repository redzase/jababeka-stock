                            ...
                            <tr class="<?php echo ($key % 2) == 0 ? "event" : "odd"; ?>">
                              ...
                              <td>
                                ...
                                <?php */ elseif ($value->status_valid == 4): // Reserved ?>
                                  ...
                                  ?>
                                  <button type='button' data-href="<?php echo site_url("sector/kavling/update_status/". $detail_sector->id ."/". $value->id ."/". STATUS_BOOKING_KAVLING_AVAILABLE); ?>" class="confirmation btn btn-default btn-xs" data-confirm-message="Anda yakin ingin mengupdate status menjadi Available?" data-heading="<?php echo $heading; ?>">Available</button>
                                  <?php 
                                  endif;
                                  ?>
                                <?php endif; ?>
        ...
        foreach ($all_data as $key => $value): 
          if ($value->offset_x > 0 and $value->offset_y > 0):
            ...
            elseif ($value->status_valid == 4): // Reserved
              ...
              if (check_access_module_permission($module, PERMISSION_AVAILABLE)):
                $content .= "<button type='button' data-href='". site_url("sector/kavling/update_status/". $detail_sector->id ."/". $value->id ."/". STATUS_BOOKING_KAVLING_AVAILABLE) ."' class='confirmation btn btn-default btn-xs' style='margin-right:10px;' data-confirm-message='Anda yakin ingin mengupdate status menjadi Available?' data-heading='". $heading ."'>Available</button>";
              endif;
