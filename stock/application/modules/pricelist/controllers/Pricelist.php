...
class Pricelist extends MY_Controller 
{
    ...
    private $_module = "STOCK_PRICELIST";
    private $_image_path = "";
    private $_image_name = "";
    ...
    public function index($page = 1) 
    {
        ...
        // if (!empty($list_sector) and !empty($year)) {
            ...
            $list_pricelist = [];
            foreach ($all_data_pricelist as $key => $value) {
                if (!isset($list_pricelist[$value->sector_id])) {
                    $list_pricelist[$value->sector_id] = [];
                }
                ...
                $list_pricelist[$value->sector_id] = array_merge($list_pricelist[$value->sector_id], [[
        ...
        $data_content["list_pricelist"]     = $list_pricelist;
	...
	private function _do_add($sector_id) 
	{
        ...
        if ($this->form_validation->run()) {
            ...
            $action = $this->Pricelistmodel->create($data_create);

            $result["status"]  = $action;
            $result["message"] = ($action) ? "Pricelist successfully created." : "Sector failed created.";
	...
	private function _do_edit($sector_id, $id) 
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $action = $this->Pricelistmodel->update($id, $data_update);

            $result["status"]  = $action;
            $result["message"] = ($action) ? "Pricelist successfully updated." : "Sector failed updated.";
	...
	public function edit($sector_id, $id)
	{
        ...
        $all_data = $this->Pricelistmodel->get_detail($params);
    ...
    {
        $action = $this->Pricelistmodel->delete($id);
    ...
    public function ajax_list_logs($sector_id, $page = 1) 
    {
        if ($this->input->is_ajax_request()) {
            ...
            $all_data = $this->Pricelistmodel->get_list_logs($params);

            $params = array(
                "get_total"  => TRUE,
                "sector_id"  => $sector_id,
                );
            $total = $this->Pricelistmodel->get_list_logs($params);  
